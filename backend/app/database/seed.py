...
def seed_db() -> None:
    ...
    try:
        ...
        first_superuser_email = (
            ...
            if "@" in settings.FIRST_SUPERUSER_USERNAME
            else None
        )
        first_superuser_username = (
            settings.FIRST_SUPERUSER_USERNAME.split("@", 1)[0]
            if first_superuser_email
            else settings.FIRST_SUPERUSER_USERNAME
        )
        existing_superuser = auth_service.get_user_by_username(
            db, first_superuser_username
        )
        if first_superuser_email and existing_superuser is None:
            existing_superuser = auth_service.get_user_by_email(
                db, first_superuser_email
            )
