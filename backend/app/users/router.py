...
def read_user_me(current_user: User = Depends(get_current_active_user)):
    """
    Get current user. Compatibility endpoint for the official frontend.
    """
    return current_user
...
def update_user_me(
    ...
    """
    Update current user. Compatibility endpoint for the official frontend.
    """
    return user_service.update_user(db=db, user=current_user, user_in=user_in)
...
def update_password_me(
    ...
    """
    Update current user's password. Compatibility endpoint for the official frontend.
    """
    updated = user_service.update_user_password(
        db=db, user=current_user, password_update=password_in
    )
    if updated is None:
        raise HTTPException(
...
def signup_user(user_in: user_schemas.UserCreate, db: Session = Depends(get_db)):
    """
    Create new user without authentication. Compatibility endpoint for the frontend.
    """
    return user_service.create_user(db=db, user_in=user_in)


@router.get("/{user_id:int}", response_model=user_schemas.UserRead)
...
@router.patch("/{user_id:int}", response_model=user_schemas.UserRead)
def update_user_by_id(
    ...
    return user_service.update_user(db=db, user=user, user_in=user_in)
